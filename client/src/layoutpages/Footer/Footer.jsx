...
export const Footer = memo(() => {
  ...
  const { data } = useSiteContent(['footer'], FALLBACK);
  const footer = data.footer || FALLBACK.footer;
  const social = Array.isArray(footer.socials) && footer.socials.length ? footer.socials : DEFAULT_SOCIAL;
  const developerTitle =
    year >= 2027
      ? footer.developerTitleFrom2027 || 'Original Lead Developer'
      : footer.developerTitleBefore2027 || 'Lead Developer';
        ...
        <section className="Footer__brand" aria-labelledby="footer-brand-heading">
          <div className="Footer__brandContainer">
            <img
              ...
              width={186}
            />
            <h2 id="footer-brand-heading" className="Footer__logo">
              {footer.brandName || 'MSP Tech Club'}
            </h2>
          </div>
          <p className="Footer__vision">
            {footer.visionLabel || 'Our Vision:'}{' '}
            <span>{footer.visionText}</span>
          </p>
        </section>
          ...
          <ul className="Footer__socialList">
            {social.map((s) => (
              <li key={s.label} className="Footer__socialItem">
                ...
                >
                  <span className="Footer__socialIcon">{ICONS[s.label] || ICONS.LinkedIn}</span>
        ...
        <section className="Footer__meta" aria-labelledby="footer-meta-heading">
          ...
          >
            ...
            <span className="Footer__developerText">
              {developerTitle}: {footer.developerName || 'Ahmed Mostafa'}
            </span>
          </a>
        </section>
      </div>
