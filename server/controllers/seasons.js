 ...
 * Body: { label: "26/27", is_default?, is_active?, sort_order? }
 */
const createSeason = async (req, res) => {
  try {
    ...
    const existing = await Season.findOne({ where: { label: parsed.label } });
    if (existing) {
    ...
    if (makeDefault) {
      await Season.update({ is_default: false }, { where: { is_default: true } });
    }
    ...
    if (!makeDefault) {
      const anyDefault = await Season.findOne({ where: { is_default: true } });
      if (!anyDefault) {
        await season.update({ is_default: true });
      }
    }
